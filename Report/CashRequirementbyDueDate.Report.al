report 50030 "Cash Req By Due Date TLY"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Local/Sales/History/CashReqByDueDateTLY.rdlc';
    Caption = 'Cash Requirement By Due Date TLY';
                    ...
                    if BeginProjectionDate > "Pmt. Discount Date" then
                        ...
                        if "Currency Code" = '' then
                            ...
                            if "Remaining Amount" <> 0 then
                                PaymentDiscToPrint := "Original Pmt. Disc. Possible" * "Remaining Amount" / "Remaining Amount"
                            else
                                PaymentDiscToPrint := 0;   // should never happen, since trx is open anyway

                    if not Vendor.Get("Vendor No.") then
                        Clear(Vendor);
                    NetRequired := "Remaining Amount" - PaymentDiscToPrint;
