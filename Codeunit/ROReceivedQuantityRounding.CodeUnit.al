Codeunit 50208 ROReceivedQuantityRounding
{
    EventSubscriberInstance = StaticAutomatic;

    [EventSubscriber(ObjectType::Page, Page::"Sales Return Order Subform", 'OnBeforeValidateEvent', 'Return Qty. Received', true, true)]
    ...
    begin
        ...
        if Rec."Return Qty. Received" = 0 then exit;
        ...
        CaseQuantity := NoOfUoM(Rec."Return Qty. Received", CaseConst);
        ...
        if Rec."Return Qty. Received" <> CaseConst * CaseQuantity then begin
            RemainingQuantity := Rec."Return Qty. Received" - (CaseConst * CaseQuantity);
            LowerQuantity := Rec."Return Qty. Received" - RemainingQuantity;
            HigherQuantity := Rec."Return Qty. Received" + (CaseConst - RemainingQuantity);
            ...
            if Selected = 2 then
                Rec."Return Qty. Received" := HigherQuantity
            else
                Rec."Return Qty. Received" := LowerQuantity;
        end;

        // Check if the quantity is larger than a pallet size and calculate the remaining quantity after converting to pallets.
        if Rec."Return Qty. Received" >= PalletConst then
            Rec."Qty. Received Pallet" := NoOfUoM(Rec."Return Qty. Received", PalletConst);

        RemainingQuantity := Rec."Return Qty. Received" - PalletConst * Rec."Qty. Received Pallet";
    ...
    [EventSubscriber(ObjectType::Page, Page::"Sales Return Order Subform", 'OnBeforeValidateEvent', 'Qty. Received Case', true, true)]
    ...
    begin
        ...
        Rec."Return Qty. Received" := (CaseConst * Rec."Qty. Received Case") + (PalletConst * Rec."Qty. Received Pallet");

        // If quantity is more than a pallet, calculate the amount for a pallet and pass remainder to case count.
        if Rec."Return Qty. Received" >= PalletConst then Rec."Qty. Received Pallet" := NoOfUoM(Rec."Return Qty. Received", PalletConst);
    ...
    [EventSubscriber(ObjectType::Page, Page::"Sales Return Order Subform", 'OnBeforeValidateEvent', 'Qty. Received Pallet', true, true)]
