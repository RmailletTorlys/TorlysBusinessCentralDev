...
report 50039 "Aged Accounts Rec Torlys"
{
    ...
    Caption = 'Aged Accounts Receivable (Extended)';
    ...
    dataset
    {
        dataitem(Header; "Integer")
        {
            ...
            // --- Existing LCY grand totals (note: this report keeps positives) ---
            ...
            // --- NEW: Customer-currency grand totals (sum of per-customer CCY amounts) ---
            column(GrandTotalBalanceDue_CustCur_; GrandTotalBalanceDueCCY) { }
            column(GrandBalanceDue_CustCur_1_; GrandBalanceDueCCY[1]) { }
            column(GrandBalanceDue_CustCur_2_; GrandBalanceDueCCY[2]) { }
            column(GrandBalanceDue_CustCur_3_; GrandBalanceDueCCY[3]) { }
            column(GrandBalanceDue_CustCur_4_; GrandBalanceDueCCY[4]) { }

            // --- NEW: Caption for RDLC grand total (customer currency)
            column(ReportTotalAmountCustomerCaption; CaptionClassTranslate('101,0,' + ReportTotalAmountCustomerLbl)) { }
            ...
            dataitem(Customer; Customer)
            {
                ...
                dataitem(Totals; "Integer")
                {
                    ...
                    begin
                        ...
                        // Bucket LCY-to-print and LCY totals
                        ...
                        // --- NEW: Compute entry in customer's currency (CCY), independent of PrintAmountsInLocal
                        GetCurrencyRecord(Currency, Customer."Currency Code");
                        AmountDueCCYToPrint :=
                            Round(
                                CurrExchRate.ExchangeAmtFCYToFCY(
                                    PeriodEndingDate[1],
                                    TempCustLedgEntry."Currency Code",   // from doc currency
                                    Customer."Currency Code",            // to customer currency ('' => LCY)
                                    TempCustLedgEntry."Remaining Amount"
                                ),
                                Currency."Amount Rounding Precision"
                            );

                        AmountDueCCY[j] := AmountDueCCYToPrint;
                        CustTotAmountDueCCY[j] := CustTotAmountDueCCY[j] + AmountDueCCYToPrint;
                        CustTotAmountDueCCYToPrint := CustTotAmountDueCCYToPrint + AmountDueCCYToPrint;

                        // End-of-customer aggregation
                        ...
                        if TotalNumberOfEntries = 0 then begin
                            ...
                            // NEW: Customer-currency grand totals
                            for j := 1 to 4 do
                                GrandBalanceDueCCY[j] += CustTotAmountDueCCY[j];
                            for j := 1 to 4 do
                                GrandTotalBalanceDueCCY += CustTotAmountDueCCY[j];
                        end;
                    ...
                    begin
                        // Keep existing Excel export body calls when not printing detail
                    ...
                    begin
                        ...
                        // --- NEW: reset CCY accumulators per customer
                        Clear(CustTotAmountDueCCY);
                        CustTotAmountDueCCYToPrint := 0;
            ...
            area(content)
            {
                group(Options)
                {
                    ...
                    field(AgedAsOf; PeriodEndingDate[1])
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Aged as of';
                        ToolTip = 'Specifies the date that aging is based on.';
                    ...
                    field(PrintAmountsInCustomersCurrency; PrintAmountsInLocal)
                    {
                        ApplicationArea = Suite;
                        Caption = 'Print Amounts in Customer''s Currency';
                        MultiLine = true;
                        ToolTip = 'Specifies if amounts are printed in the customer''s currency.';
                    }
    ...
    begin
        ...
        // --- NEW: Ensure customer-currency grand totals start at zero
        Clear(GrandBalanceDueCCY);
        GrandTotalBalanceDueCCY := 0;
    end;
    ...
    var
        ...
        // --- NEW VARIABLES: customer-currency accumulators
        GrandBalanceDueCCY: array[4] of Decimal;
        GrandTotalBalanceDueCCY: Decimal;
        CustTotAmountDueCCY: array[4] of Decimal;
        CustTotAmountDueCCYToPrint: Decimal;
        AmountDueCCY: array[4] of Decimal;
        AmountDueCCYToPrint: Decimal;
        ReportTotalAmountCustomerLbl: Label 'Report Total Amount Due (Customer Currency)';
    ...
    // --- base procedures unchanged unless noted ---
    ...
    begin
        ...
        ExcelBuf.AddInfoColumn(REPORT::"Aged Accounts Rec Torlys", false, false, false, false, '', ExcelBuf."Cell Type"::Number);
        ExcelBuf.NewRow();
    ...
    // --- NEW: Cloud-safe Excel creation (replaces OnPrem CreateBookAndOpenExcel)
    ...
    begin
        ...
        FileName := StrSubstNo('%1_%2', AgedAccountsReceivableLbl, Format(Today(), 0, 9));
