report 50038 "Aged Accounts Payable Torlys"
{
    ...
    Caption = 'Aged Accounts Payable (Extended)';
    ...
    dataset
    {
        dataitem(Header; "Integer")
        {
            ...
            // --- NEW: Vendor-currency grand totals (sum of per-vendor vendor-currency amounts) ---
            column(GrandTotalBalanceDue_VendorCur_; -GrandTotalBalanceDueVCY) { }
            column(GrandBalanceDue_VendorCur_1_; -GrandBalanceDueVCY[1]) { }
            column(GrandBalanceDue_VendorCur_2_; -GrandBalanceDueVCY[2]) { }
            column(GrandBalanceDue_VendorCur_3_; -GrandBalanceDueVCY[3]) { }
            column(GrandBalanceDue_VendorCur_4_; -GrandBalanceDueVCY[4]) { }

            // Optional caption for RDLC
            column(ReportTotalAmountVendorCaption; CaptionClassTranslate('101,0,' + ReportTotalAmountVendorLbl)) { }
            ...
            dataitem(Vendor; Vendor)
            {
                ...
                dataitem("Vendor Ledger Entry"; "Vendor Ledger Entry")
                {
                    ...
                    begin
                        ...
                        CurrReport.Skip(); // Defer to Totals dataitem
                    end;
                ...
                dataitem(Totals; "Integer")
                {
                    ...
                    // --- Base dataset columns (unchanged) ---
                    column(AmountDueToPrint; -AmountDueToPrint) { AutoFormatExpression = TempVendorLedgerEntry."Currency Code"; AutoFormatType = 1; }
                    column(AmountDue_1_; -AmountDue[1]) { AutoFormatExpression = TempVendorLedgerEntry."Currency Code"; AutoFormatType = 1; }
                    column(AmountDue_2_; -AmountDue[2]) { AutoFormatExpression = TempVendorLedgerEntry."Currency Code"; AutoFormatType = 1; }
                    column(AmountDue_3_; -AmountDue[3]) { AutoFormatExpression = TempVendorLedgerEntry."Currency Code"; AutoFormatType = 1; }
                    column(AmountDue_4_; -AmountDue[4]) { AutoFormatExpression = TempVendorLedgerEntry."Currency Code"; AutoFormatType = 1; }
                    ...
                    column(AmountDueToPrint_Control63; -AmountDueToPrint) { AutoFormatExpression = TempVendorLedgerEntry."Currency Code"; AutoFormatType = 1; }
                    column(AmountDue_1__Control64; -AmountDue[1]) { AutoFormatExpression = TempVendorLedgerEntry."Currency Code"; AutoFormatType = 1; }
                    column(AmountDue_2__Control65; -AmountDue[2]) { AutoFormatExpression = TempVendorLedgerEntry."Currency Code"; AutoFormatType = 1; }
                    column(AmountDue_3__Control66; -AmountDue[3]) { AutoFormatExpression = TempVendorLedgerEntry."Currency Code"; AutoFormatType = 1; }
                    column(AmountDue_4__Control67; -AmountDue[4]) { AutoFormatExpression = TempVendorLedgerEntry."Currency Code"; AutoFormatType = 1; }

                    column(Vendor_Ledger_Entry___Currency_Code_; TempVendorLedgerEntry."Currency Code") { }

                    column(AmountDueToPrint_Control68; -AmountDueToPrint) { AutoFormatExpression = TempVendorLedgerEntry."Currency Code"; AutoFormatType = 1; }
                    column(AmountDue_1__Control69; -AmountDue[1]) { AutoFormatExpression = TempVendorLedgerEntry."Currency Code"; AutoFormatType = 1; }
                    column(AmountDue_2__Control70; -AmountDue[2]) { AutoFormatExpression = TempVendorLedgerEntry."Currency Code"; AutoFormatType = 1; }
                    column(AmountDue_3__Control71; -AmountDue[3]) { AutoFormatExpression = TempVendorLedgerEntry."Currency Code"; AutoFormatType = 1; }
                    column(AmountDue_4__Control72; -AmountDue[4]) { AutoFormatExpression = TempVendorLedgerEntry."Currency Code"; AutoFormatType = 1; }

                    column(AmountDueToPrint_Control74; -AmountDueToPrint) { AutoFormatExpression = TempVendorLedgerEntry."Currency Code"; AutoFormatType = 1; }
                    column(AmountDue_1__Control75; -AmountDue[1]) { AutoFormatExpression = TempVendorLedgerEntry."Currency Code"; AutoFormatType = 1; }
                    column(AmountDue_2__Control76; -AmountDue[2]) { AutoFormatExpression = TempVendorLedgerEntry."Currency Code"; AutoFormatType = 1; }
                    column(AmountDue_3__Control77; -AmountDue[3]) { AutoFormatExpression = TempVendorLedgerEntry."Currency Code"; AutoFormatType = 1; }
                    column(AmountDue_4__Control78; -AmountDue[4]) { AutoFormatExpression = TempVendorLedgerEntry."Currency Code"; AutoFormatType = 1; }
                    ...
                    column(AmountDueToPrint_Control81; -AmountDueToPrint) { AutoFormatExpression = TempVendorLedgerEntry."Currency Code"; AutoFormatType = 1; }
                    column(AmountDue_1__Control82; -AmountDue[1]) { AutoFormatExpression = TempVendorLedgerEntry."Currency Code"; AutoFormatType = 1; }
                    column(AmountDue_2__Control83; -AmountDue[2]) { AutoFormatExpression = TempVendorLedgerEntry."Currency Code"; AutoFormatType = 1; }
                    column(AmountDue_3__Control84; -AmountDue[3]) { AutoFormatExpression = TempVendorLedgerEntry."Currency Code"; AutoFormatType = 1; }
                    column(AmountDue_4__Control85; -AmountDue[4]) { AutoFormatExpression = TempVendorLedgerEntry."Currency Code"; AutoFormatType = 1; }
                    ...
                    begin
                        ...
                        if TempVendorLedgerEntry."Currency Code" <> '' then
                            TempVendorLedgerEntry."Remaining Amt. (LCY)" :=
                                Round(
                                    CurrencyExchangeRate.ExchangeAmtFCYToFCY(
                                        PeriodEndingDate[1],
                                        TempVendorLedgerEntry."Currency Code",
                                        '',
                                        TempVendorLedgerEntry."Remaining Amount"
                                    )
                                );
                        ...
                        // --- NEW: Compute vendor-currency amount regardless of PrintAmountsInLocal ---
                        GetCurrencyRecord(Currency, Vendor."Currency Code"); // establishes rounding precision
                        AmountDueVCYToPrint :=
                            Round(
                                CurrencyExchangeRate.ExchangeAmtFCYToFCY(
                                    PeriodEndingDate[1],
                                    TempVendorLedgerEntry."Currency Code", // document currency
                                    Vendor."Currency Code",                // vendor currency ('' => LCY)
                                    TempVendorLedgerEntry."Remaining Amount"
                                ),
                                Currency."Amount Rounding Precision"
                            );
                        AmountDueVCY[j] := AmountDueVCYToPrint;
                        VendTotAmountDueVCY[j] := VendTotAmountDueVCY[j] + AmountDueVCYToPrint;
                        VendTotAmountDueVCYToPrint := VendTotAmountDueVCYToPrint + AmountDueVCYToPrint;

                        // Track when we've processed the last entry for this vendor
                        ...
                        if TotalNumberOfEntries = 0 then begin
                            ...
                            // NEW: Vendor-currency grand totals
                            for j := 1 to 4 do
                                GrandBalanceDueVCY[j] += VendTotAmountDueVCY[j];
                            for j := 1 to 4 do
                                GrandTotalBalanceDueVCY += VendTotAmountDueVCY[j];
                        end;
                    ...
                    begin
                        ...
                        // --- NEW: Reset vendor-currency accumulators for this vendor
                        Clear(VendTotAmountDueVCY);
                        VendTotAmountDueVCYToPrint := 0;
            ...
            area(content)
            {
                group(Options)
                {
                    ...
                    field(PrintAmountsInVendorsCurrency; PrintAmountsInLocal)
                    {
                        ...
                        MultiLine = true;
                        ToolTip = 'Specifies if amounts are printed in the vendor''s currency. Clear the check box to print all amounts in your functional currency.';
    ...
    begin
        ...
        // Ensure vendor-currency grand totals start at zero for each run
        Clear(GrandBalanceDueVCY);
        GrandTotalBalanceDueVCY := 0;
    end;
    ...
    var
        ...
        // --- NEW VARIABLES: Vendor-currency accumulators ---
        GrandBalanceDueVCY: array[4] of Decimal;
        GrandTotalBalanceDueVCY: Decimal;
        VendTotAmountDueVCY: array[4] of Decimal;
        VendTotAmountDueVCYToPrint: Decimal;
        AmountDueVCY: array[4] of Decimal;
        AmountDueVCYToPrint: Decimal;
        ReportTotalAmountVendorLbl: Label 'Report Total Amount Due (Vendor Currency)';
    ...
    begin
        ...
        TempExcelBuffer.AddInfoColumn(REPORT::"Aged Accounts Payable Torlys", false, false, false, false, '', TempExcelBuffer."Cell Type"::Number);
        TempExcelBuffer.NewRow();
    ...
    var
        FileName: Text;
    begin
        // Create the workbook and write the current buffer content to a sheet
        TempExcelBuffer.CreateNewBook(DataLbl); // first sheet name (e.g., 'Data')
        TempExcelBuffer.WriteSheet(AgedAccountsPayableLbl, CompanyName(), UserId());
        TempExcelBuffer.CloseBook();

        // Suggest a nice download name and open (download) the file
        FileName := StrSubstNo('%1_%2', AgedAccountsPayableLbl, Format(Today(), 0, 9));
        TempExcelBuffer.SetFriendlyFilename(FileName);
        TempExcelBuffer.OpenExcel();
    end;

    local procedure GetVendorCurrencyForCaption(VendorCurrencyCode: Code[10]): Text[30]
    begin
        // Return a short display token for the caption
        if VendorCurrencyCode <> '' then
            exit(VendorCurrencyCode); // or use Currency.Description if you prefer full names
