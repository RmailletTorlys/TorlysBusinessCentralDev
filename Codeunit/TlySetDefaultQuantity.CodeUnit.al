codeunit 50017 TlySetDefaultQuantity
{
    ...
    begin
        ...
        // GetSalesSetup(); //TLY-SD - 03/27/2026 - don't need this, is pointless
        // SalesSetup.Get(); //TLY-SD - 03/27/2026 - don't need this, is pointless
        // if (SalesSetup."Default Quantity to Ship" = SalesSetup."Default Quantity to Ship"::Blank) then begin //TLY-SD - 03/27/2026 - don't need this, is pointless
        //TLY-SD - 06/17/2026 - start - needed in conjuction with InitQtyToShip2 to not allocate orders that are partial ship
        SalesHeader.Reset;
        SalesHeader.SetRange("No.", SalesLine."Document No.");
        if SalesHeader.Find('-') then begin
            // Message('%1 --- %2 --- %3', SalesHeader."No.", SalesHeader.Ship, SalesHeader.Invoice);
            if (SalesHeader.Ship = false) and (SalesHeader.Invoice = false) then begin
                //TLY-SD - 06/17/2026 - end - needed in conjuction with InitQtyToShip2 to not allocate orders that are partial ship
                if (SalesLine."Document Type" = SalesLine."Document Type"::Order) or (SalesLine."Document Type" = SalesLine."Document Type"::Quote) then begin
                    if SalesLine."Type" = SalesLine."Type"::Item then begin
                        ...
                        if ((SalesLine."Shipment Date" - WorkDate()) >= 0) and ((SalesLine."Shipment Date" - WorkDate()) <= 45) then begin
                            ...
                            // Message('Round 3. QTS = %1', SalesLine."Qty. to Ship");
                        end else begin
