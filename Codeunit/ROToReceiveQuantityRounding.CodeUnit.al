codeunit 50207 ROToReceiveQuantityRounding
{
    ...
    begin
        ...
        if Rec."Return Qty. to Receive" = 0 then exit;
        ...
        CaseQuantity := NoOfUoM(Rec."Return Qty. to Receive", CaseConst);
        ...
        if Rec."Return Qty. to Receive" <> CaseConst * CaseQuantity then begin
            RemainingQuantity := Rec."Return Qty. to Receive" - (CaseConst * CaseQuantity);
            LowerQuantity := Rec."Return Qty. to Receive" - RemainingQuantity;
            HigherQuantity := Rec."Return Qty. to Receive" + (CaseConst - RemainingQuantity);
            ...
            if Selected = 2 then
                Rec."Return Qty. to Receive" := HigherQuantity
            else
                Rec."Return Qty. to Receive" := LowerQuantity;
        end;
        ...
        if Rec."Return Qty. to Receive" >= PalletConst then
            Rec."Qty. to Receive Pallet" := NoOfUoM(Rec."Return Qty. to Receive", PalletConst);
    ...
    begin
        ...
        if Rec."Return Qty. to Receive" >= PalletConst then Rec."Qty. to Receive Pallet" := NoOfUoM(Rec."Return Qty. to Receive", PalletConst);
