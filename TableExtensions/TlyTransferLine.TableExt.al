tableextension 55741 TlyTransferLine extends "Transfer Line"
{
    fields
    {
    ...
    begin
        ...
        if Rec."Qty. in Transit" <> 0 then begin
            Error('This is in-transit, so cannot be deleted!');
        end else begin
            if Rec."Sales Order No." <> '' then begin
                ...
                if not AllowChange then begin
                    Error('Will not be deleted. %1 with a quantity of %2 is joined to %3 line %4.', Rec."Item No.", Rec.Quantity, Rec."Sales Order No.", Rec."Sales Order Line No.");
                end else begin
                    SalesLine.Reset;
